...
import { PaginationSchema, PaginationSchemaTransform, BasePaginationSchemaOutput } from './pagination'
...
describe('PaginationSchema', () => {
  ...
  it('should accept custom offset and limit values', () => {
    const result = PaginationSchema.parse({ offset: 5, limit: 20 })
...
describe('PaginationSchemaTransform', () => {
  ...
  it('should cap limit at 100', () => {
    const result = PaginationSchemaTransform.parse({ offset: 0, limit: 150 })
    expect(result.limit).toBe(100)
  })

  it('should set limit to 10 if a non-positive number is provided', () => {
    const result = PaginationSchemaTransform.parse({ offset: 0, limit: -5 })
    expect(result.limit).toBe(10)
  })

  it('should transform string numbers into numbers', () => {
    const result = PaginationSchemaTransform.parse({ offset: '5', limit: '30' })
    expect(result.offset).toBe(5)
...
describe('BasePaginationSchemaOutput', () => {
  it('should have default values for meta', () => {
    const result = BasePaginationSchemaOutput.parse({ values: [] })
