...
export const PaginationSchema = z
  .object({
    offset: z.coerce.number().int().default(0).meta({ description: 'Number of items to skip in the result set.', example: 10, default: 0 }),
    limit: z.coerce.number().int().default(10).meta({ description: 'Maximum number of items to return.', example: 20, default: 10 }),
  })
  .partial()
...
const PaginationMeta = z.object({
  total: z.number().int().default(0).meta({ example: 150 }),
  offset: z.number().int().default(0).meta({ example: 20 }),
  limit: z.number().int().default(0).meta({ example: 10 }),
})
