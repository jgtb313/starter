...
export type IUserRepository = {
  findAllPaginated(input: Pagination<User>): Promise<PaginationOutput<UserDomain>>
  findAll(input: Partial<User>): Promise<UserDomain[]>
  findById(userId: string): Promise<UserDomain>
  findByEmail(email: string, options?: { workspaceId?: User['workspaceId'] }): Promise<UserDomain | null>
  findByPhone(phone: Phone, options?: { workspaceId?: User['workspaceId'] }): Promise<UserDomain | null>
