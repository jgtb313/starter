import { BasePaymentCard, BasePix, BaseBoleto } from '@starter/schema'
import { uuid } from '@starter/common'

import { RecurrencePaymentMethodEnum } from '@/ports/recurrence'
import { InvoiceDomain } from '@/core/invoice/invoice.domain'
import { InvoiceInput, InvoiceStatusEnum } from '@/core/invoice/invoice.schema'

type InvoiceOverrides =
  | (Partial<Omit<InvoiceInput, 'workspaceId' | 'subscriptionId'>> &
      Pick<InvoiceInput, 'workspaceId' | 'subscriptionId'> & { paymentMethod: RecurrencePaymentMethodEnum.CARD; card: BasePaymentCard })
  | (Partial<Omit<InvoiceInput, 'workspaceId' | 'subscriptionId'>> &
      Pick<InvoiceInput, 'workspaceId' | 'subscriptionId'> & { paymentMethod: RecurrencePaymentMethodEnum.PIX; pix: BasePix })
  | (Partial<Omit<InvoiceInput, 'workspaceId' | 'subscriptionId'>> &
...
export const makeInvoice = (overrides: InvoiceOverrides): InvoiceDomain => {
  const base: Omit<InvoiceInput, 'workspaceId' | 'subscriptionId' | 'paymentMethod'> = {
    invoiceId: uuid(),
    externalId: uuid(),
