...
describe('InvoiceService', () => {
  ...
  describe('InvoiceService', () => {
    describe('getPaginatedInvoices', () => {
      it.each([
        { input: { offset: 0, limit: 10 }, length: 6, total: 6 },
        { input: { offset: 0, limit: 2 }, length: 2, total: 6 },
      ])('should return paginated invoices correctly', async ({ input, length, total }) => {
    ...
    describe('getInvoice', () => {
      ...
      it('should throw an error if the invoice does not exist', async () => {
        expect(() => service.getInvoice('invalid-invoiceId')).rejects.toThrow(new NotFoundException('Invoice invalid-invoiceId not found'))
      })
    ...
    describe('createInvoice', () => {
      ...
      it('should call getWorkspace and getSubscription with correct IDs', async () => {
        ...
        expect(workspaceServiceMock.getWorkspace).toBeCalledWith('0e6c34bb-5a5c-4b31-bfec-33ec3651d57f')
        expect(subscriptionServiceMock.getSubscription).toBeCalledWith('126b6b16-0238-41bc-9c27-54f260b08aaa')
      })
    })
    ...
    describe('updateInvoice', () => {
      ...
      it('should throw an error if the invoice does not exist', async () => {
        expect(() => service.updateInvoice('invalid-invoiceId', {})).rejects.toThrow(new NotFoundException('Invoice invalid-invoiceId not found'))
      })
    })
  })
})
