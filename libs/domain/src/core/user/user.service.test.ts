...
describe('UserService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [InMemoryDatabaseModule.register(), UserRepositoryModule],
      providers: [
        UserService,
        EncryptModule,
  ...
  describe('getUser', () => {
    it('should return user if belongs to workspace', async () => {
      ...
      const result = await service.getUser({ userId: user.state.userId, workspaceId: `${user.state.workspaceId}` })
      expect(result.state.userId).toBe(user.state.userId)
    })

    it('should throw AclForbiddenException if workspaceId mismatches', async () => {
      const [user] = userMocks
      await expect(service.getUser({ userId: user.state.userId, workspaceId: 'wrong-id' })).rejects.toThrow(AclForbiddenException)
    })
  })
