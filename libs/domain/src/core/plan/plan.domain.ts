...
import { type Plan, PlanSchema, PlanStatusEnum } from '@/core/plan/plan.schema'
...
export class PlanDomain extends BaseDomain<Plan, PlanInput> {
	...
	nextBillingDate(lastBillingDate: Date) {
		const intervals = {
			[RecurrenceIntervalEnum.DAY]: addDays(
				new Date(lastBillingDate),
				this.state.intervalCount,
			),
			[RecurrenceIntervalEnum.WEEK]: addWeeks(
				new Date(lastBillingDate),
				this.state.intervalCount,
			),
			[RecurrenceIntervalEnum.MONTH]: addMonths(
				new Date(lastBillingDate),
				this.state.intervalCount,
			),
			[RecurrenceIntervalEnum.YEAR]: addYears(
				new Date(lastBillingDate),
				this.state.intervalCount,
			),
		}
	...
	markAsDeleted() {
		if (this.isDeleted()) {
			throw new ConflictException(
				`Plan ${this.state.planId} is already deleted`,
			)
		}
