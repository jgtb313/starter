...
describe('WorkspaceDomain', () => {
  it('should return true for isActive when status is ACTIVE', () => {
    const workspace = makeWorkspace({ status: WorkspaceStatusEnum.ACTIVE })
    expect(workspace.isActive()).toBe(true)
  })

  it('should return false for isActive when status is INACTIVE', () => {
    const workspace = makeWorkspace({ status: WorkspaceStatusEnum.INACTIVE })
  ...
  it('should throw when marking workspace as active if already active', () => {
    const workspace = makeWorkspace({ status: WorkspaceStatusEnum.ACTIVE })
    expect(() => workspace.markAsActive()).toThrowError(new ConflictException('This workspace is already active.'))
  })
  ...
  it('should throw when marking workspace as inactive if already inactive', () => {
    const workspace = makeWorkspace({ status: WorkspaceStatusEnum.INACTIVE })
    expect(() => workspace.markAsInactive()).toThrowError(new ConflictException('This workspace is already inactive.'))
  })
})
