...
describe('WorkspaceService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [InMemoryDatabaseModule.register(), WorkspaceRepositoryModule],
      providers: [
        WorkspaceService,
        {
          provide: UserService,
  ...
  describe('getPaginatedWorkspaces', () => {
    it.each([
      { input: { offset: 0, limit: 10 }, length: 5, total: 5 },
      { input: { offset: 0, limit: 1 }, length: 1, total: 5 },
    ])('should return paginated workspaces correctly', async ({ input, length, total }) => {
  ...
  describe('getWorkspace', () => {
    ...
    it('should throw NotFoundException if workspace does not exist', async () => {
      await expect(service.getWorkspace('non-existent')).rejects.toThrow(new NotFoundException('Workspace non-existent not found'))
    })
  })
  ...
  describe('updateWorkspace', () => {
    it('should update workspace correctly', async () => {
      ...
      const result = await service.updateWorkspace(workspace.state.workspaceId, {
    ...
    it('should throw NotFoundException if workspace not found', async () => {
      await expect(service.updateWorkspace('non-existent', { name: 'X' })).rejects.toThrow(new NotFoundException('Workspace non-existent not found'))
    })
  })
  ...
  describe('activeWorkspace', () => {
    ...
    it('should throw ConflictException if already active', async () => {
      ...
      await expect(service.activeWorkspace(workspace.state.workspaceId)).rejects.toThrow(new ConflictException('This workspace is already active.'))
    })
  })
  ...
  describe('inactiveWorkspace', () => {
    it('should mark workspace as inactive', async () => {
      ...
      const result = await service.inactiveWorkspace(workspace.state.workspaceId)
    ...
    it('should throw ConflictException if already inactive', async () => {
      ...
      await expect(service.inactiveWorkspace(workspace.state.workspaceId)).rejects.toThrow(
