...
export const loadDatabase = async (module: TestingModule) => {
  ...
  await dataSource.getRepository(PermissionEntity).insert(permissionMocks.map((permission) => permission.toJSON()))
  await dataSource.getRepository(OrganizationEntity).insert(organizationMocks.map((organization) => organization.toJSON()))
  ...
  await dataSource.getRepository(RoleOrganizationEntity).insert(roleOrganizations)
