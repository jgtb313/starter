...
export class RoleTypeorm implements IRoleRepository {
	...
	validateRoleIds: IRoleRepository['validateRoleIds'] = async (roleIds) => {
		...
		if (missingRoleIds.length) {
			throw new NotFoundException(
				`The following roleIds were not found: ${missingRoleIds.join(', ')}`,
			)
		}
	}
	...
	private toRoleEntity({
		...
		permissions,
		...role
	}: BaseRole): DeepPartial<RoleEntity> {
		return {
			...role,
		}
	}
	...
	private toPartialRoleEntity({
		...
		permissions,
		...role
	}: Partial<BaseRole>): QueryDeepPartialEntity<RoleEntity> {
		return {
			...role,
		}
	}
