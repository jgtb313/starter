...
describe('MobilePushStrategy', () => {
  ...
  describe('send', () => {
    it('should send a hello notification successfully', async () => {
      const notification: Parameters<INotificationStrategy<'MOBILE_PUSH'>['send']>[0] = {
        recipient: 'user123',
        template: 'HELLO',
        props: { userName: 'John' },
      }
      ...
      expect(templates.templates.HELLO).toHaveBeenCalledWith({ userName: 'John' })
    ...
    it('should handle errors from mobilePush.send', async () => {
      ...
      await expect(mobilePushService.send(notification)).rejects.toThrow('Push notification failed')
      expect(templates.templates.HELLO).toHaveBeenCalledWith({ userName: 'John' })
      expect(mobilePushMock.send).toHaveBeenCalledWith({
        to: 'user123',
        body: 'Hello, John',
