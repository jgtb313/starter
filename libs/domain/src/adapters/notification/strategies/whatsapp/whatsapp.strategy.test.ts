...
describe('WhatsappStrategy', () => {
  ...
  describe('send', () => {
    it('should send an OTP notification successfully', async () => {
      const notification: Parameters<INotificationStrategy<'WHATSAPP'>['send']>[0] = {
      ...
      expect(templates.templates.SEND_OTP).toHaveBeenCalledWith({ code: '123456' })
    ...
    it('should handle errors from whatsapp.send', async () => {
      ...
      await expect(whatsappService.send(notification)).rejects.toThrow('Whatsapp notification failed')
      expect(templates.templates.SEND_OTP).toHaveBeenCalledWith({ code: '123456' })
      expect(whatsappMock.send).toHaveBeenCalledWith({
        to: '1234567890',
        contentSid: 'HX1234567890',
