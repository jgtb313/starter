...
const ErrorSchema = {
	400: () =>
		z.object({
			...
			issues: z.array(z.record(z.string(), z.string())).meta({
				example: [
					{
...
const isHttpResponseError = (
	value: HttpStatus,
): value is HttpStatusErrorResponses =>
	[
		'400',
		'401',
		'403',
...
export const Route = (options: RouteOptions): MethodDecorator => {
	return (target, propertyKey, descriptor) => {
		...
		const guards =
			reflector.get(
				GUARDS_METADATA,
				target.constructor.prototype[propertyKey as keyof typeof target],
			) ?? []
												...
												value:
													'schema' in item
														...
														: isHttpResponseError(httpResponse)
															? sample(
																	zodSchemaToJSONSchema(
																		ErrorSchema[httpResponse](item.description),
																	),
																)
															: {},
											},
