...
import { FileContextEnum } from './file.service'

export const GetPresignedUrlBodySchema = z.object({
  filename: z.string().openapi({ example: 'avatar.png' }),
  context: z.nativeEnum(FileContextEnum),
})
export const GetPresignedUrlSchemaOutput = z.object({
  filename: z.string().openapi({ example: 'https://static.example.com.br/users/avatars/avatar.png' }),
  filenameSigned: z.string().openapi({
    example:
...
export type GetPresignedUrlBodyInput = z.infer<typeof GetPresignedUrlBodySchema>
